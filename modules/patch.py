...
# import comfy.modules.args_parser
        ...
        if patch_type == "diff":
            ...
            if alpha != 0.0:
                if w1.shape != weight.shape:
                    print("形状不匹配 {} FOOOCUS权重未合并 {} != {}".format(key, w1.shape, weight.shape))
                else:
            ...
            print("patch类型无法识别", patch_type, key)
...
def patched_cldm_forward(self, x, hint, timesteps, context, y=None, **kwargs):
    t_emb = comfy.ldm.modules.diffusionmodules.openaimodel.timestep_embedding(timesteps, self.model_channels, repeat_only=False).to(x.dtype)
...
def patched_load_models_gpu(*args, **kwargs):
    ...
    if moving_time > 0.1:
        print(f'[Fooocus 模型管理] 移动模型用时 {moving_time:.2f} 秒')
    return y
...
def patch_all():
    ...
    comfy.samplers.KSamplerX0Inpaint.forward = patched_KSamplerX0Inpaint_forward
